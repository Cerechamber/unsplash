...
const Popup = ({ photoId }) => {
  console.log(photoId, 666);
  const navigate = useNavigate();
  const popup = useRef();

  const currentPhoto = useSelector((state) => {
    const popupPhoto = state.photosReducer.photos.find(
      ({ id }) => id === photoId,
    );
    return popupPhoto;
  });
  ...
  const closePopup = (ev) => {
    if (
      !ev.target.closest('.popup__self') ||
      ev.target.classList.contains('.popup__close')
    ) {
    ...
    <>
      {photoId ? (
        <div className='popup' onClick={closePopup} ref={popup}>
          <div className='popup__close' title='Закрыть'>
            X
          </div>
          <div className='popup__self'>
                ...
                <div className='popup__texts'>
                  <div className='popup__texts1'>
                    <div className='popup__author-link'>
                      ...
                      <div>
                        <a
                          href={
                            currentPhoto.href +
                            '?utm_source=react_photo_app&utm_medium=referral'
                          }
                          target='_blank'
                          rel='nofollow noreferrer'
                        >
                    ...
                    <div className='popup__date'>
                      {currentPhoto.date.split('T')[0]}
                    </div>
                  </div>
                  {currentPhoto.description ? (
                    <p className='popup__descr'>{currentPhoto.description}</p>
                  ) : null}
                </div>
              </div>
            </div>
            <div className='popup__bottom'>
              <div className='popup__likes'>
                <LikeButton id={currentPhoto.id} liked={currentPhoto.liked} />
